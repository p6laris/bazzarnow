...
<head>
    <meta charset="utf-8" />
    <title>BazzarNow – Edit Product</title>
    ...
    <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
        <a href="{{ route('admin.products.index') }}" class="flex items-center gap-3 group">
            ...
            <div class="flex flex-col leading-tight">
                ...
                <span class="text-[11px] uppercase tracking-[0.22em] text-neutral-500">Edit product</span>
            </div>
        </a>
    </div>
</header>
...
<main class="max-w-3xl mx-auto px-4 py-10 space-y-6">
    <h1 class="text-2xl font-semibold tracking-tight mb-2">Edit product</h1>
    ...
    <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="space-y-1 text-sm">
            ...
            <input name="name" value="{{ old('name', $product->name) }}" class="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-3 py-2 text-sm focus:border-neutral-200 outline-none">
        </div>

        <div class="space-y-1 text-sm">
            <label class="text-neutral-300">Description</label>
            <textarea name="description" rows="3" class="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-3 py-2 text-sm focus:border-neutral-200 outline-none">{{ old('description', $product->description) }}</textarea>
        </div>

        <div class="grid gap-4 sm:grid-cols-3 text-sm">
            <div class="space-y-1">
                <label class="text-neutral-300">Price (USD)</label>
                <input type="number" step="0.01" name="price" value="{{ old('price', $product->price / 100) }}" class="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-3 py-2 text-sm focus:border-neutral-200 outline-none">
            </div>
            <div class="space-y-1">
                ...
                <select name="category" class="w-full rounded-lg bg-neutral-950 border border-neutral-800 px-3 py-2 text-sm focus:border-neutral-200 outline-none">
                    @foreach ($categories as $cat)
                        <option value="{{ $cat }}" @selected(old('category', $product->category) === $cat)>
                            {{ ucfirst($cat) }}
                        </option>
                    @endforeach
                </select>
        ...
        @if ($product->image_url)
            <div class="space-y-2">
                <p class="text-xs text-neutral-400">Current image:</p>
                <div class="h-40 w-40 rounded-xl overflow-hidden border border-neutral-800 bg-neutral-900">
